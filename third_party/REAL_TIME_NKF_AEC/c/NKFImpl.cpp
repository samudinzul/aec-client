...
int NKFImpl::Enhance(std::string in_audio, std::string lpb_audio, std::string out_audio) {
    ...
    for (int i = 0; i < process_num; i++) {
        memmove(m_pEngine.mic_buffer, m_pEngine.mic_buffer + BLOCK_SHIFT,
                (BLOCK_LEN - BLOCK_SHIFT) * sizeof(float));
        memmove(m_pEngine.lpb_buffer, m_pEngine.lpb_buffer + BLOCK_SHIFT,
                (BLOCK_LEN - BLOCK_SHIFT) * sizeof(float));
        ...
        for (int j = 0; j < BLOCK_SHIFT; j++) {
            testdata.push_back(m_pEngine.out_buffer[j]);
        }
    }
...
// ============================================================
void NKFImpl::ProcessBlock(const float* mic_new, const float* lpb_new, float* out) {
    memmove(m_pEngine.mic_buffer, m_pEngine.mic_buffer + BLOCK_SHIFT,
            (BLOCK_LEN - BLOCK_SHIFT) * sizeof(float));
    memmove(m_pEngine.lpb_buffer, m_pEngine.lpb_buffer + BLOCK_SHIFT,
            (BLOCK_LEN - BLOCK_SHIFT) * sizeof(float));
    ...
    for (int n = 0; n < BLOCK_SHIFT; n++) {
        m_pEngine.mic_buffer[n + BLOCK_LEN - BLOCK_SHIFT] = mic_new[n];
        m_pEngine.lpb_buffer[n + BLOCK_LEN - BLOCK_SHIFT] = lpb_new[n];
    }

    OnnxInfer();

    for (int j = 0; j < BLOCK_SHIFT; j++) {
        out[j] = m_pEngine.out_buffer[j];
    }
}
...
void NKFImpl::OnnxInfer() {
    ...
    memmove(m_pEngine.lpb_imag, m_pEngine.lpb_imag + FFT_OUT_SIZE,
            (NKF_LEN - 1) * FFT_OUT_SIZE * sizeof(float));
    ...
    for (int i = 0; i < FFT_OUT_SIZE; i++) {
        for (int j = 0; j < NKF_LEN; j++) {
            ...
            e_real[i] += m_pEngine.lpb_real[j * FFT_OUT_SIZE + i] * m_pEngine.h_prior_real[NKF_LEN * i + j]
                       - m_pEngine.lpb_imag[j * FFT_OUT_SIZE + i] * m_pEngine.h_prior_imag[NKF_LEN * i + j];
            e_imag[i] += m_pEngine.lpb_real[j * FFT_OUT_SIZE + i] * m_pEngine.h_prior_imag[NKF_LEN * i + j]
                       + m_pEngine.lpb_imag[j * FFT_OUT_SIZE + i] * m_pEngine.h_prior_real[NKF_LEN * i + j];
        }
    ...
    for (int i = 0; i < FFT_OUT_SIZE; i++) {
        for (int j = 0; j < NKF_LEN; j++) {
            m_pEngine.h_posterior_real[NKF_LEN * i + j] =
                m_pEngine.h_prior_real[NKF_LEN * i + j]
                + e_real[i] * kg_real[NKF_LEN * i + j]
                - e_imag[i] * kgimag[NKF_LEN * i + j];
            m_pEngine.h_posterior_imag[NKF_LEN * i + j] =
                m_pEngine.h_prior_imag[NKF_LEN * i + j]
                + e_imag[i] * kg_real[NKF_LEN * i + j]
    ...
    for (int i = 0; i < FFT_OUT_SIZE; i++) {
        for (int j = 0; j < NKF_LEN; j++) {
            echohat_real[i] += m_pEngine.lpb_real[j * FFT_OUT_SIZE + i] * m_pEngine.h_posterior_real[NKF_LEN * i + j]
                             - m_pEngine.lpb_imag[j * FFT_OUT_SIZE + i] * m_pEngine.h_posterior_imag[NKF_LEN * i + j];
            echohat_imag[i] += m_pEngine.lpb_real[j * FFT_OUT_SIZE + i] * m_pEngine.h_posterior_imag[NKF_LEN * i + j]
